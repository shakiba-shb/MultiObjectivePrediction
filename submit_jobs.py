...
parser.add_argument('-S', action = 'store', dest='Ss', type=str, default = '100,200,300,400,500', help='Population size')
parser.add_argument('-dim', action = 'store', dest='dims', type=str, default = '5,15,25,50,75', help='Number of objectives/variables')
...
parser.add_argument('-seed', action = 'store', dest='seeds', type=str, default = '14724,24284,31658,6933,1318,16695,27690,8233,24481,6832', help='Random seed')
...
parser.add_argument('-time', action='store', dest='time', default='48:00:00', type=str, help='time in HR:MN:SS')
...
all_commands = []
job_info = []
rdir = '/'.join([args.rdir])+'/'+algs[0]+'/'
os.makedirs(rdir, exist_ok=True)

for alg,s,dim,damp,seed,epsilon,epsilon_type,n_gen,diagnostic_id,l in it.product(algs,Ss,dims,damps,seeds,eps,ep_types,n_gens,diagnostic_ids,Ls):

	all_commands.append(
		f'python /mnt/home/shahban1/MultiObjectivePrediction/single_comparison_experiment.py -alg {alg} -S {int(s)} -dim {int(dim)} -damp {float(damp)} -epsilon {float(epsilon)} -epsilon_type {int(epsilon_type)} -seed {int(seed)} -rdir {rdir} -n_gen {int(n_gen)} -diagnostic_id {int(diagnostic_id)} -L {int(l)}'
	)
