...
for s,dim,damp,seed,epsilon,epsilon_type,n_gen,diagnostic_id,l in it.product(Ss,dims,damps,seeds,eps,ep_types,n_gens,diagnostic_ids,Ls):

	all_commands.append(
		f'python /mnt/home/shahban1/MultiObjectivePrediction/single_comparison_experiment.py -S {int(s)} -dim {int(dim)} -damp {float(damp)} -epsilon {float(epsilon)} -epsilon_type {int(epsilon_type)} -seed {int(seed)} -rdir {rdir} -n_gen {int(n_gen)} -diagnostic_id {int(diagnostic_id)} -L {int(l)}'
	)
...
if args.slurm:
	...
	for i, run_cmd in enumerate(all_commands):
		...
		job_name = '_'.join([x + '-' + f'{job_info[i][x]}' for x in
							['S','dim', 'damp', 'epsilon', 'epsilon_type', 'seed']])
